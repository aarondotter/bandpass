module bandpass
  ...
  type spectrum
     ...
     real(dp) :: redshift = 0d0, vrot = 0d0
     real(dp), allocatable :: wave(:), flux(:), extinction(:)
  end type spectrum

contains

  subroutine rotational_broadening(s,ierr)
    type(spectrum), intent(inout) :: s
    integer, intent(out) :: ierr
    double precision :: sigma_inv, sigma
    double precision, allocatable :: lnwave(:), weight(:), x(:), flux(:)
    !fit to R. Kurucz's rotate.for
    double precision, parameter :: n_sigma=1.5d0
    double precision, parameter :: factor=0.65d0
    double precision, parameter :: native_res = 1.5d1 !km/s C3K SEDs
    integer :: i, j, n
    if(s% vrot < native_res) return
    ierr=0
    n=s% npts
    sigma = sqrt( s% vrot*s% vrot - native_res*native_res)
    allocate(lnwave(n),weight(n),x(n),flux(n))
    sigma_inv = clight/AAkm/factor/sigma
    lnwave=log(s% wave)
    flux=s% flux
    do i=10,n-10
       weight=0d0
       x=(lnwave-lnwave(i))*sigma_inv
       do j=1,n
          if(x(j) < -n_sigma) then
             cycle
          elseif(x(j) < n_sigma) then
             weight(j) = exp(-0.5d0*x(j)*x(j))
          else
             exit
          endif
       enddo
       flux(i) = tsum(lnwave,weight*s% flux)/tsum(lnwave,weight)
    enddo
    s% flux = flux
  end subroutine rotational_broadening


  subroutine set_redshift(type,val,s,ierr)
    integer, intent(in) :: type
    real(dp), intent(in) :: val
    type(spectrum), intent(inout) :: s
    integer, intent(out) :: ierr
    real(dp) :: beta
    !first translate to redshift z
    if(type==redshift_z)then
       s% redshift = val
       ierr=0
    elseif(type==velocity_km_s)then !expect velocity in km/s
       !radial velocity only, not transverse
       beta=val*AAkm/clight
       s% redshift = sqrt((1d0+beta)/(1d0-beta)) - 1d0
       ierr=0
    else
       s% redshift=0d0
       ierr=-1
  ...
  subroutine write_spectrum(s,out,ierr)
    type(spectrum), intent(in) :: s
    character(len=256), intent(in) :: out
    integer, intent(out) :: ierr
    integer :: i
    ierr=0
    write(*,*) 'writing ', trim(s% filename), ' to ', trim(out)
    open(1,file=trim(out),iostat=ierr,action='write')
    if(ierr/=0) return
    write(1,'(a1,i10,5f12.3)') '#', s% npts, s% Teff, s% logg, s% FeH, s% vrot, s% redshift
    do i=1,s% npts
       write(1,'(1p3e20.10)') s% wave(i), s% flux(i), s% extinction(i)
    enddo
    close(1)
